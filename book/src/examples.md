...
| Example | Features | Shows |
|---|---|---|
| [`hot_reload`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/hot_reload.rs) | `watch`, `toml` | Edit a file and watch the snapshot follow. |
| [`async_reload`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/async_reload.rs) | `async`, `watch`, `json` | A task awaiting reloads instead of polling for them. |
| [`group`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/group.rs) | `json` | Two config types reloading as one step, or not at all. |

## Getting it right

| Example | Features | Shows |
|---|---|---|
| [`validation`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/validation.rs) | `json` | Rejecting a configuration where every field is valid and the whole is not. |
| [`secrets`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/secrets.rs) | `json` | `#[config(secret)]`, and precisely what it does and does not cover. |
| [`testing`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/testing.rs) | `json` | Pinning configuration under test with the override layer. |

## With a web framework

| Example | Features | Shows |
|---|---|---|
| [`axum_hello`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/axum_hello.rs) | `watch`, `json` | A handler that reads `current()` per request, a `/config/check` probe, and why the listen port is start-up configuration. |
| [`actix_hello`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/actix_hello.rs) | `watch`, `json` | The same across Actix's worker threads, and why configuration does not belong in `web::Data`. |

## On a runtime

| Example | Features | Shows |
|---|---|---|
| [`tokio_runtime`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/tokio_runtime.rs) | `tokio`, `watch`, `json` | Two readers each waking on their own `changes()` handle, with tokio's blocking pool wired in for free. |
| [`smol_runtime`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/smol_runtime.rs) | `async`, `watch`, `json` | The whole async surface on smol, with smol's `unblock` installed as the blocking executor and no tokio in the build. |
| [`embassy_runtime`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/embassy_runtime.rs) | `async`, `json` | Embassy — an executor for microcontrollers, with no threads and no reactor — driving `changes()`, and why two rapid reloads are one wakeup. |

## Reaching further

| Example | Features | Shows |
|---|---|---|
| [`units`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/units.rs) | `json` | `"30s"` and `"64MiB"`, from files and from the environment. |
...
| [`remote`](https://github.com/ctolon/dynamic-config/blob/main/dynamic-config/examples/remote.rs) | `json` | A `RemoteSource` of your own: explicit fetch, where it sits between the layers, an unreachable store, and a watch loop pushing through its sink. |
