//! End-to-end behaviour of `#[dynamic_config]` and its builder.
//!
...
fn db_builder() -> Builder<DbConfig> {
    DbConfig::builder("db")
...
fn server_builder() -> Builder<ServerConfig> {
...
fn later_files_win_and_tables_merge_rather_than_replace() {
    let config = db_builder().load().expect("fixtures should deserialize");
...
fn the_snapshot_appears_on_init_and_swaps_on_replace() {
    ...
    db_builder().init().expect("init should succeed");
...
fn a_missing_file_is_skipped_rather_than_failing() {
    ...
    let error = MissingConfig::builder("missing")
