...
| `promote.sh` | `dev` → `main`: pushes, opens the pull request if it is not already open, arms auto-merge, waits for the gates, merges (rebase), and re-syncs `dev` onto the new `main`. Picks up wherever `propose.sh` stopped. |
